    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if(commandSender instanceof Player){
            ...
            if(strings.length == 0){
                ...
                //player.sendMessage(Data.prefix + "§7Benutze §8× §e/tpahere <Spieler>");
                ...
                if(target != null){
                    if(target != player){
                        if(MySQL.getTpaToggle(target.getUniqueId().toString()) == 1){
                            TPAHere.put(player, target);
                            TPAHere.put(target, player);

                            player.sendMessage(Data.prefix + "§7Du hast eine §cTPA-Anfrage §7an §6" + Data.getPlayerPrefix(target) + " §6gesendet.§7.");
                            target.sendMessage(Data.prefix + Data.getPlayerPrefix(player) + " §7möchte gerne, dass du dich zu ihm teleportierst.");
